    ...
    fn set_textures(
        ...
        for (tex_id, delta) in textures_delta_set {
            ...
            let (pixels, size) = match delta.image {
                egui_backend::egui::ImageData::Color(pixel_colors) => {
                    let pixels: Vec<u8> = pixel_colors.pixels
                        
                        .into_iter().flat_map(|color32| {
                            color32.to_array()
                        }).collect();
                    (pixels, pixel_colors.size)
                },
                ...
                egui_backend::egui::TextureId::Managed(tex_id) => {
                    if let Some(_) = delta.pos {
                        ...
                        let mip_level_count = if tex_id == 0 {
                            ...
                            1//panic!("get mip map count formula")
                        };
